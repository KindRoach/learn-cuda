#include "sharedMemoryVectorSum.cuh"
...
__global__ void sumVectorOnGPU_SharedMemory_Ver2(float *vec, float *res) {
    ...
    for (unsigned i = 2; i <= blockDim.x; i *= 2) {
        if (threadIdx.x < blockDim.x / i) {
            ...
            smem[idx] = smem[idx] + smem[idx + i / 2];
        }
        __syncthreads();
    }
...
void sharedMemoryVectorSum(size_t nElement, size_t nThread) {
    ...
    size_t nSharedBytes = nThread * sizeof(float);
    auto ver1_func = [&]() { sumVectorOnGPU_SharedMemory_Ver1<<<nBlock, nThread, nSharedBytes>>>(d_vec, d_result); };
    auto ver2_func = [&]() { sumVectorOnGPU_SharedMemory_Ver2<<<nBlock, nThread, nSharedBytes>>>(d_vec, d_result); };
    auto ver3_func = [&]() { sumVectorOnGPU_SharedMemory_Ver3<<<nBlock, nThread, nSharedBytes>>>(d_vec, d_result); };
